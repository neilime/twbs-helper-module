---
id: toasts
title: Toasts
---

import ReactHtmlParser from 'react-html-parser';
import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

[Twitter bootstrap Documentation](https://getbootstrap.com/docs/4.5/components/toasts/)
## Examples
[Twitter bootstrap Documentation](https://getbootstrap.com/docs/4.5/components/toasts/#examples)
### Basic
[Twitter bootstrap Documentation](https://getbootstrap.com/docs/4.5/components/toasts/#basic)
<Tabs
    defaultValue="result"
    values={[
      {label: 'Result', value: 'result'},
      {label: 'Source', value: 'source'},
    ]}>
<TabItem value="result">{ ReactHtmlParser(`<div aria-atomic="true" aria-live="assertive" class="toast" role="alert">
    <div class="toast-header">
        <img alt="..." class="mr-2&#x20;rounded" src="images&#x2F;demo&#x2F;rounded-blue.svg" />
        <strong class="mr-auto">Bootstrap</strong>
        <small class="text-muted">11 mins ago</small>
        <button aria-label="Close" class="close&#x20;mb-1&#x20;ml-2" data-dismiss="toast" type="button"><span aria-hidden="true">&times;</span></button>
    </div>
    <div class="toast-body">
        Hello, world! This is a toast message.
    </div>
</div>`) }</TabItem>
<TabItem value="source">

```php
echo $this->toast([
    'header' => [
        'image' => [
            'images/demo/rounded-blue.svg',
            ['alt' => '...', 'class' => 'rounded mr-2'],
        ],
        'title' => 'Bootstrap',
        'subtitle' => '11 mins ago',
    ],
    'body' => 'Hello, world! This is a toast message.',
]);
```

</TabItem>
</Tabs>

### Translucent
[Twitter bootstrap Documentation](https://getbootstrap.com/docs/4.5/components/toasts/#translucent)
<Tabs
    defaultValue="result"
    values={[
      {label: 'Result', value: 'result'},
      {label: 'Source', value: 'source'},
    ]}>
<TabItem value="result">{ ReactHtmlParser(`<div class="bg-dark"><div aria-atomic="true" aria-live="assertive" class="toast" role="alert">
    <div class="toast-header">
        <img alt="..." class="mr-2&#x20;rounded" src="images&#x2F;demo&#x2F;rounded-blue.svg" />
        <strong class="mr-auto">Bootstrap</strong>
        <small class="text-muted">11 mins ago</small>
        <button aria-label="Close" class="close&#x20;mb-1&#x20;ml-2" data-dismiss="toast" type="button"><span aria-hidden="true">&times;</span></button>
    </div>
    <div class="toast-body">
        Hello, world! This is a toast message.
    </div>
</div></div>`) }</TabItem>
<TabItem value="source">

```php
echo '<div class="bg-dark">';

echo $this->toast([
    'header' => [
        'image' => [
            'images/demo/rounded-blue.svg',
            ['alt' => '...', 'class' => 'rounded mr-2'],
        ],
        'title' => 'Bootstrap',
        'subtitle' => '11 mins ago',
    ],
    'body' => 'Hello, world! This is a toast message.',
]);

echo '</div>';
```

</TabItem>
</Tabs>

### Stacking
[Twitter bootstrap Documentation](https://getbootstrap.com/docs/4.5/components/toasts/#stacking)
<Tabs
    defaultValue="result"
    values={[
      {label: 'Result', value: 'result'},
      {label: 'Source', value: 'source'},
    ]}>
<TabItem value="result">{ ReactHtmlParser(`<div aria-atomic="true" aria-live="assertive" class="toast" role="alert">
    <div class="toast-header">
        <img alt="..." class="mr-2&#x20;rounded" src="images&#x2F;demo&#x2F;rounded-blue.svg" />
        <strong class="mr-auto">Bootstrap</strong>
        <small class="text-muted">just now</small>
        <button aria-label="Close" class="close&#x20;mb-1&#x20;ml-2" data-dismiss="toast" type="button"><span aria-hidden="true">&times;</span></button>
    </div>
    <div class="toast-body">
        See? Just like this.
    </div>
</div>
<div aria-atomic="true" aria-live="assertive" class="toast" role="alert">
    <div class="toast-header">
        <img alt="..." class="mr-2&#x20;rounded" src="images&#x2F;demo&#x2F;rounded-blue.svg" />
        <strong class="mr-auto">Bootstrap</strong>
        <small class="text-muted">2 seconds ago</small>
        <button aria-label="Close" class="close&#x20;mb-1&#x20;ml-2" data-dismiss="toast" type="button"><span aria-hidden="true">&times;</span></button>
    </div>
    <div class="toast-body">
        Heads up, toasts will stack automatically
    </div>
</div>`) }</TabItem>
<TabItem value="source">

```php
echo $this->toast([
    'header' => [
        'image' => [
            'images/demo/rounded-blue.svg',
            ['alt' => '...', 'class' => 'rounded mr-2'],
        ],
        'title' => 'Bootstrap',
        'subtitle' => 'just now',
    ],
    'body' => 'See? Just like this.',
]) . PHP_EOL;

echo $this->toast([
    'header' => [
        'image' => [
            'images/demo/rounded-blue.svg',
            ['alt' => '...', 'class' => 'rounded mr-2'],
        ],
        'title' => 'Bootstrap',
        'subtitle' => '2 seconds ago',
    ],
    'body' => 'Heads up, toasts will stack automatically',
]);
```

</TabItem>
</Tabs>

### Placement
[Twitter bootstrap Documentation](https://getbootstrap.com/docs/4.5/components/toasts/#placement)
<Tabs
    defaultValue="result"
    values={[
      {label: 'Result', value: 'result'},
      {label: 'Source', value: 'source'},
    ]}>
<TabItem value="result">{ ReactHtmlParser(`<div class="bg-dark" style="position:relative;min-height:200px;"><div aria-atomic="true" aria-live="assertive" class="toast" role="alert" style="position&#x3A;&#x20;absolute&#x3B;right&#x3A;&#x20;0&#x3B;top&#x3A;&#x20;0&#x3B;">
    <div class="toast-header">
        <img alt="..." class="mr-2&#x20;rounded" src="images&#x2F;demo&#x2F;rounded-blue.svg" />
        <strong class="mr-auto">Bootstrap</strong>
        <small class="text-muted">11 mins ago</small>
        <button aria-label="Close" class="close&#x20;mb-1&#x20;ml-2" data-dismiss="toast" type="button"><span aria-hidden="true">&times;</span></button>
    </div>
    <div class="toast-body">
        Hello, world! This is a toast message.
    </div>
</div></div>
<br/><br/>
<div class="bg-dark" style="position:relative;min-height:200px;"><div aria-atomic="true" aria-live="assertive" class="toast" role="alert" style="left&#x3A;&#x20;0&#x3B;margin-left&#x3A;&#x20;auto&#x3B;margin-right&#x3A;&#x20;auto&#x3B;position&#x3A;&#x20;absolute&#x3B;right&#x3A;&#x20;0&#x3B;top&#x3A;&#x20;0&#x3B;">
    <div class="toast-header">
        <img alt="..." class="mr-2&#x20;rounded" src="images&#x2F;demo&#x2F;rounded-blue.svg" />
        <strong class="mr-auto">Bootstrap</strong>
        <small class="text-muted">11 mins ago</small>
        <button aria-label="Close" class="close&#x20;mb-1&#x20;ml-2" data-dismiss="toast" type="button"><span aria-hidden="true">&times;</span></button>
    </div>
    <div class="toast-body">
        Hello, world! This is a toast message.
    </div>
</div></div>`) }</TabItem>
<TabItem value="source">

```php
echo '<div class="bg-dark" style="position:relative;min-height:200px;">';

echo $this->toast([
    'placement' => 'top-right',
    'header' => [
        'image' => [
            'images/demo/rounded-blue.svg',
            ['alt' => '...', 'class' => 'rounded mr-2'],
        ],
        'title' => 'Bootstrap',
        'subtitle' => '11 mins ago',
    ],
    'body' => 'Hello, world! This is a toast message.',
]);

echo '</div>';

echo PHP_EOL . '<br/><br/>' . PHP_EOL;

echo '<div class="bg-dark" style="position:relative;min-height:200px;">';

echo $this->toast([
    'placement' => 'top-center',
    'header' => [
        'image' => [
            'images/demo/rounded-blue.svg',
            ['alt' => '...', 'class' => 'rounded mr-2'],
        ],
        'title' => 'Bootstrap',
        'subtitle' => '11 mins ago',
    ],
    'body' => 'Hello, world! This is a toast message.',
]);

echo '</div>';
```

</TabItem>
</Tabs>

### Accessibility
[Twitter bootstrap Documentation](https://getbootstrap.com/docs/4.5/components/toasts/#accessibility)
<Tabs
    defaultValue="result"
    values={[
      {label: 'Result', value: 'result'},
      {label: 'Source', value: 'source'},
    ]}>
<TabItem value="result">{ ReactHtmlParser(`<div aria-atomic="true" aria-live="assertive" class="toast" data-autohide="false" role="alert">
    <div class="toast-header">
        <img alt="..." class="mr-2&#x20;rounded" src="images&#x2F;demo&#x2F;rounded-blue.svg" />
        <strong class="mr-auto">Bootstrap</strong>
        <small class="text-muted">11 mins ago</small>
        <button aria-label="Close" class="close&#x20;mb-1&#x20;ml-2" data-dismiss="toast" type="button"><span aria-hidden="true">&times;</span></button>
    </div>
    <div class="toast-body">
        Hello, world! This is a toast message.
    </div>
</div>`) }</TabItem>
<TabItem value="source">

```php
echo $this->toast([
    'autohide' => false,
    'header' => [
        'image' => [
            'images/demo/rounded-blue.svg',
            ['alt' => '...', 'class' => 'rounded mr-2'],
        ],
        'title' => 'Bootstrap',
        'subtitle' => '11 mins ago',
    ],
    'body' => 'Hello, world! This is a toast message.',
]);
```

</TabItem>
</Tabs>

